    ...
    for (let i = 0; i < userList.length; i++) {
        ...
        if (taskList) {
            if (Priority == 'High') {
                ...
                await sendEmail(userList[i], users[userList[i]], notificationTemplate.getNotificationTemplate_Medium(userList[i], taskList));
            }
            else {
                await sendEmail(userList[i], users[userList[i]], notificationTemplate.getNotificationTemplate_Low(userList[i], taskList));
            }
        }
    }
}
